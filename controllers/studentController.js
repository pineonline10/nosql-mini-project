...
const headCount = async () => {
  ...
  return numberOfStudents[0] ? numberOfStudents[0].totalStudents : 0;
};
...
const grade = async (studentId) =>
  Student.aggregate([
    ...
    // TODO: Group information for the student with the given ObjectId alongside an overall grade calculated using the $avg operator
    {
      ...
      $group: {
        _id: "$_id",
        avgGrade: { $avg: "$assignments.grade" }
      }
    },
  ]);
...
module.exports = {
  ...
  async getStudents(req, res) {
    try {
      ...
      const numberOfStudents = await headCount();
      const totalheadCount = numberOfStudents;
      const studentObj = {
        students,
